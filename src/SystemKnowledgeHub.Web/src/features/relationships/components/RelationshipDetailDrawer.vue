...
import { ArrowRight, Close, DocumentChecked, EditPen, Plus, Refresh } from '@element-plus/icons-vue'
...
async function saveDescription(){if(!detail.value||saving.value)return;saving.value=true;errorMessage.value=null;try{const result=await updateRelationshipDescription(detail.value.id,{description:description.value.trim()||null,actor:actorStore.actor,concurrencyToken:detail.value.concurrencyToken});detail.value={...detail.value,description:result.description,concurrencyToken:result.concurrencyToken};editing.value=false;window.dispatchEvent(new CustomEvent('relationship:changed'));ElMessage.success('关系说明已更新。')}catch(e:unknown){conflict.value=e instanceof ApiError&&e.status===409;errorMessage.value=e instanceof Error?e.message:'关系说明保存失败。'}finally{saving.value=false}}
async function changeStatus(){if(!detail.value||!nextStatus.value||!requirementMet.value||saving.value)return;saving.value=true;errorMessage.value=null;try{await changeRelationshipStatus(detail.value.id,{targetStatus:nextStatus.value,reason:null,actor:{displayName:actorStore.displayName,roleOrIdentity:actorStore.role??'知识整理人员',occurredAt:new Date().toISOString(),team:null,externalUserKey:null,source:'System Knowledge Hub',note:null},concurrencyToken:detail.value.concurrencyToken});await load();window.dispatchEvent(new CustomEvent('relationship:changed'));ElMessage.success('关系知识状态已明确推进。')}catch(e:unknown){conflict.value=e instanceof ApiError&&e.status===409;errorMessage.value=e instanceof Error?e.message:'知识状态修改失败。'}finally{saving.value=false}}
function addEvidence(){if(!detail.value)return;overlayStore.openDrawer({kind:'add-evidence',id:detail.value.id,mode:'create',payload:{subject:{type:'KnowledgeRelation',id:detail.value.id},title:`${detail.value.source.title} → ${detail.value.target.title}`,knowledgeStatus:detail.value.knowledgeStatus,subjectDetailKey:null}})}
...
<template><div class="relationship-drawer relationship-detail-drawer"><LoadingState v-if="loading&&!detail" message="正在读取关系详情…"/><ErrorState v-else-if="errorMessage&&!detail" title="关系详情加载失败" :message="errorMessage" @retry="load"/><template v-else-if="detail">
  <header><el-button text circle :icon="Close" aria-label="关闭关系详情" @click="overlayStore.closeDrawer()"/><span>关系详情</span><h2>{{ relationTypeLabels[detail.relationType] }}</h2><p>显式知识关系</p></header>
  ...
  <section><div class="relationship-heading"><h3>关系说明</h3><el-button v-if="!editing" text type="primary" :icon="EditPen" @click="editing=true">编辑</el-button></div><p v-if="!editing" class="relationship-description">{{ detail.description??'尚未记录关系说明。' }}</p><template v-else><el-input v-model="description" type="textarea" :rows="3" maxlength="500"/><div class="relationship-edit-actions"><el-button @click="editing=false;description=detail.description??''">取消</el-button><el-button type="primary" :loading="saving" @click="saveDescription">保存说明</el-button></div></template></section>
  <section class="relationship-status"><div class="relationship-heading"><h3>知识状态</h3><KnowledgeStatusBadge :status="detail.knowledgeStatus"/></div><KnowledgeProgression :status="detail.knowledgeStatus"/><p>证据是关系依据，保存后不会自动改变状态。</p><el-button v-if="nextStatus&&!statusConfirming" type="primary" plain size="small" @click="statusConfirming=true">推进为{{ nextStatus==='Inferred'?'推断':'已确认' }}</el-button><div v-if="statusConfirming" class="relationship-status-confirm"><strong>{{ requirementMet?'推进条件已满足':'暂时不能推进' }}</strong><p>{{ detail.knowledgeStatus==='Unknown'?'需要至少一条可定位的关系证据。':'需要至少一条完整的人工确认证据。' }}</p><div><el-button size="small" @click="statusConfirming=false">取消</el-button><el-button size="small" type="primary" :disabled="!requirementMet" :loading="saving" @click="changeStatus">明确推进</el-button></div></div></section>
  <section><div class="relationship-heading"><h3>证据 <b>{{ detail.evidence.length }}</b></h3><el-button text type="primary" :icon="Plus" @click="addEvidence">添加证据</el-button></div><div v-if="detail.evidence.length" class="relationship-evidence"><button v-for="item in detail.evidence" :key="item.id" @click="openEvidence(item.id)"><el-icon><DocumentChecked/></el-icon><span><small>{{ evidenceTypeLabel(item.evidenceType) }}</small><strong>{{ item.sourceTitle }}</strong></span><el-icon><ArrowRight/></el-icon></button></div><div v-else class="relationship-empty"><el-icon><DocumentChecked/></el-icon> 尚未添加支持这条关系的证据。</div></section>
  ...
  <footer><p>本阶段不支持修改源对象、目标对象和关系类型。</p><el-button @click="overlayStore.closeDrawer()">关闭</el-button></footer>
