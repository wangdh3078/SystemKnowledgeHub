...
describe('SystemUnifiedKnowledgeView', () => {
  ...
  it('shows aggregated counts, documents and bounded section content', async () => {
    ...
    expect(wrapper.text()).toContain('HTTP API · 单向')
    expect(wrapper.text()).toContain('操作规程 · 已发布')
    expect(wrapper.text()).toContain('代码引用')
    expect(wrapper.text()).toContain('高优先级 · 待处理')
    expect(wrapper.text()).not.toContain('CodeReference')
