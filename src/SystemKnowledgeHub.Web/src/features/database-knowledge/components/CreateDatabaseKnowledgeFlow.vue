...
const sourceOptions = ref<
  readonly { readonly id: number; readonly name: string; readonly engine: string }[]
>([])
...
<template>
  <Teleport v-if="overlayStore.isDialogOpen" defer to="#dialog-feature-content">
    <section
      v-if="kind === 'create-database-knowledge'"
      class="create-database-knowledge-dialog"
      aria-labelledby="create-database-knowledge-title"
    >
      <header class="authoring-header">
        <div>
          <h2 id="create-database-knowledge-title">新增数据库知识</h2>
          <p>先登记实际数据库来源，或手工登记一个表或视图；字段在对象详情中后续补充。</p>
        </div>
        <button
          class="authoring-close"
          type="button"
          aria-label="关闭"
      ...
      <div class="database-create-choice-grid">
        <button
          type="button"
          class="database-create-choice"
          @click="
            overlayStore.openDialog({ kind: 'create-database-source', id: null, mode: 'create' })
          "
        >
        ...
        <button
          ...
          class="database-create-choice"
          @click="
            overlayStore.openDialog({ kind: 'register-database-object', id: null, mode: 'create' })
          "
        >
          <el-icon :size="24"><Plus /></el-icon>
          <strong>登记数据库对象</strong>
          <span>在已有数据库来源下手工登记表或视图。</span>
          <el-icon :size="17"><ArrowLeft class="database-create-choice__arrow" /></el-icon>
        </button>
      </div>
      <p class="database-create-choice-note">
        最小信息保存后，业务说明、字段、关系和证据均可在后续上下文中渐进补充。
      </p>
    </section>

    <section
      v-else-if="kind === 'create-database-source'"
      class="create-database-form-dialog"
      aria-labelledby="create-database-source-title"
    >
      <header class="authoring-header authoring-header--form">
        <div>
          <button class="authoring-back" type="button" @click="goBack">
            <el-icon><ArrowLeft /></el-icon>选择数据库知识类型
          </button>
      ...
      >
        ...
        <el-form-item label="所属系统" prop="systemId" :error="fieldErrors.systemId" required>
          <el-select v-model="sourceForm.systemId" filterable placeholder="选择已登记系统"
            ><el-option
        ...
        <div class="create-database-form-dialog__row">
          <el-form-item label="数据库来源名称" prop="name" :error="fieldErrors.name" required
            ><el-input
              v-model="sourceForm.name"
              class="technical-input"
              placeholder="例如 MES 生产库"
          /></el-form-item>
          <el-form-item label="数据库类型" prop="engine" :error="fieldErrors.engine" required
            ><el-input
              v-model="sourceForm.engine"
              class="technical-input"
              placeholder="例如 Oracle"
          /></el-form-item>
        </div>
        <el-collapse>
          <el-collapse-item title="补充来源信息（可选）" name="optional-source">
            <div class="create-database-form-dialog__row">
              <el-form-item label="环境"
                ><el-input v-model="sourceForm.environment" placeholder="例如 Production"
              /></el-form-item>
              <el-form-item label="实例名称"
                ><el-input v-model="sourceForm.instanceName" class="technical-input"
              /></el-form-item>
              <el-form-item label="服务名"
                ><el-input v-model="sourceForm.serviceName" class="technical-input"
              /></el-form-item>
              <el-form-item label="数据库名"
                ><el-input v-model="sourceForm.databaseName" class="technical-input"
              /></el-form-item>
            </div>
      ...
      <footer class="authoring-actions">
        <p>
          创建人：{{ actorStore.displayName }}{{ actorStore.role ? ` · ${actorStore.role}` : '' }}
        </p>
        <div>
          <el-button @click="overlayStore.closeDialog">取消</el-button
          ><el-button type="primary" :loading="submitting" @click="submitSource"
            >登记数据库来源</el-button
          >
        </div>
      </footer>
    ...
    <section
      v-else-if="kind === 'register-database-object'"
      class="create-database-form-dialog"
      aria-labelledby="register-database-object-title"
    >
      <header class="authoring-header authoring-header--form">
        <div>
          <button class="authoring-back" type="button" @click="goBack">
          ...
          <p>仅登记表或视图的最小元数据；不会自动导入字段或推进知识状态。</p>
        </div>
        <button
          class="authoring-close"
          type="button"
      ...
      >
        ...
        <el-form-item
          label="数据库来源"
          prop="databaseSourceId"
          :error="fieldErrors.databaseSourceId"
          required
        >
          <el-select v-model="objectForm.databaseSourceId" filterable placeholder="选择数据库来源"
            ><el-option
        ...
        <div class="create-database-form-dialog__row">
          <el-form-item
            ...
            required
            ><el-input
              v-model="objectForm.schemaName"
              class="technical-input"
              placeholder="例如 MES"
          /></el-form-item>
          <el-form-item label="对象名称" prop="objectName" :error="fieldErrors.objectName" required
            ><el-input
              v-model="objectForm.objectName"
              class="technical-input"
              placeholder="例如 TABLE_EQP"
          /></el-form-item>
          <el-form-item label="对象类型" prop="objectType" :error="fieldErrors.objectType" required
            ><el-select v-model="objectForm.objectType"
        ...
        <el-collapse>
          <el-collapse-item title="补充对象元数据（可选）" name="optional-object">
            <div class="create-database-form-dialog__row">
              <el-form-item label="估算行数（可选）" :error="fieldErrors.estimatedRows"
                ><el-input-number
                  v-model="objectForm.estimatedRows"
                  :min="0"
                  :precision="0"
                  controls-position="right"
              /></el-form-item>
              <el-form-item label="读写方式" :error="fieldErrors.accessMode"
                ><el-select v-model="objectForm.accessMode"
                  ><el-option label="待确认" value="Unknown" /><el-option
                    label="只读"
                    value="Read" /><el-option label="只写" value="Write" /><el-option
                    label="读 / 写"
                    value="ReadWrite" /></el-select
              ></el-form-item>
              <el-form-item label="主键字段"
                ><el-input
                  v-model="objectForm.primaryKeyColumns"
                  class="technical-input"
                  placeholder="EQP_ID，多个用逗号分隔"
              /></el-form-item>
              <el-form-item label="业务唯一键"
                ><el-input
                  v-model="objectForm.businessKeyColumns"
                  class="technical-input"
                  placeholder="EQP_CODE，多个用逗号分隔"
              /></el-form-item>
            </div>
            <el-form-item label="业务说明"
              ><el-input
                v-model="objectForm.businessDescription"
                type="textarea"
                :rows="2"
                maxlength="500"
                show-word-limit
                placeholder="可后续在对象详情中补充"
            /></el-form-item>
          </el-collapse-item>
        </el-collapse>
      </el-form>
      <footer class="authoring-actions">
        ...
        <div>
          <el-button @click="overlayStore.closeDialog">取消</el-button
          ><el-button type="primary" :loading="submitting" @click="submitObject"
            >登记对象</el-button
          >
        </div>
      </footer>
