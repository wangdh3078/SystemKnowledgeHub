...
function notifyColumnChanged(): void {
  window.dispatchEvent(new Event('database-column:changed'))
}
...
function startEditing(): void {
  ...
  knownValueForm.sortOrder =
    detail.value.knownValues.length === 0
      ? 0
      : Math.max(...detail.value.knownValues.map((item) => Number(item.value) || 0)) + 1
...
async function saveBusinessKnowledge(): Promise<void> {
  ...
  editError.value = null
  try {
    await updateDatabaseColumnKnowledge(detail.value.id, {
      businessDescription: businessDescription.value.trim() || null,
    ...
    editError.value = mutationError(error, '字段业务知识保存失败。')
  } finally {
    saving.value = false
  }
}
...
async function addKnownValue(): Promise<void> {
  ...
  if (!knownValueForm.value.trim() || !knownValueForm.meaning.trim()) {
    editError.value = '请填写已知值及其业务含义。'
    return
  }
  knownValueSubmitting.value = true
  editError.value = null
  try {
    await addColumnKnownValue(detail.value.id, {
      value: knownValueForm.value.trim(),
      meaning: knownValueForm.meaning.trim(),
      sortOrder: knownValueForm.sortOrder,
      actor: actorStore.actor,
      concurrencyToken: detail.value.concurrencyToken,
    })
    await reload()
    ...
    knownValueForm.meaning = ''
    resetDrawerDirty()
    if (businessDescription.value !== (detail.value?.businessKnowledge.description ?? ''))
      markDrawerDirty()
    ElMessage.success('已添加字段已知值，知识状态未自动改变。')
  } catch (error: unknown) {
    editError.value = mutationError(error, '添加已知值失败。')
  } finally {
    knownValueSubmitting.value = false
  }
}
...
async function removeKnownValue(knownValueId: number, value: string): Promise<void> {
  ...
  try {
    await ElMessageBox.confirm(
      `确认移除已知值“${value}”吗？如果它已被证据或开放待确认事项精确引用，系统会阻止移除。`,
      '移除已知值',
  ...
  editError.value = null
  try {
    await removeColumnKnownValue(detail.value.id, knownValueId, {
      confirmed: true,
      actor: actorStore.actor,
      concurrencyToken: detail.value.concurrencyToken,
    })
    await reload()
    notifyColumnChanged()
    ElMessage.success('已移除未被引用的已知值。')
  } catch (error: unknown) {
    editError.value = mutationError(error, '移除已知值失败。')
  } finally {
    knownValueSubmitting.value = false
  }
}
...
<template>
  <div class="column-drawer">
    ...
    <template v-else-if="detail">
      ...
      <el-collapse v-model="activeSections" class="column-drawer__sections">
          ...
          ><template #title
            ><div class="drawer-collapse-title">
              ...
              ><el-button
                ...
                >编辑</el-button
              ><span v-else-if="editing" class="drawer-editing-label">正在编辑</span>
            </div></template
          ><el-form v-if="editing" label-position="top" class="drawer-edit-form" @submit.prevent
            ><el-form-item label="业务说明"
              ><el-input
                ...
                maxlength="500"
                show-word-limit
                placeholder="说明字段的业务含义"
            /></el-form-item>
            <p class="drawer-section-note">数据库元数据、知识状态、证据和关系不会在此修改。</p>
            <div class="drawer-edit-actions">
              <el-button @click="stopEditing">取消</el-button
              ><el-button type="primary" :loading="saving" @click="saveBusinessKnowledge"
                >保存业务知识</el-button
              >
            </div></el-form
          ><template v-else
            ...
            <p class="drawer-section-note">业务含义与支撑它的证据分开保存。</p></template
          ></el-collapse-item
        >
        ...
        <el-collapse-item name="unknownItems"
          ><template #title
            ><div class="drawer-collapse-title">
              <span
                >待确认事项 <b>{{ detail.unknownItems.length }}</b></span
              ><el-button
                v-if="actorStore.canEdit"
                text
                type="primary"
                :icon="Plus"
                @click.stop="createUnknownItem"
                >添加</el-button
              >
            </div></template
          >
          <div v-if="detail.unknownItems.length" class="drawer-unknown-list">
            <article v-for="item in detail.unknownItems" :key="item.id">
              ...
              <div>
                ...
                ><span>{{ unknownStatusLabel(item.status) }}</span>
              </div>
            </article>
          </div>
        ...
        <el-collapse-item name="knownValues"
          ><template #title
            ><div class="drawer-collapse-title">
              <span
                >已知值 <b>{{ detail.knownValues.length }}</b></span
              ><el-button
                v-if="editing"
                text
                type="primary"
                :icon="Plus"
                @click.stop="addKnownValue"
                >添加值</el-button
              >
            </div></template
          >
          <div v-if="detail.knownValues.length" class="known-values-list">
            <div v-for="item in detail.knownValues" :key="item.id">
              ...
              ><el-button
                ...
                type="danger"
                :icon="Delete"
                :disabled="knownValueSubmitting"
                @click="removeKnownValue(item.id, item.value)"
                >移除</el-button
              >
            </div>
          </div>
          ...
          <el-form v-if="editing" class="known-value-editor" inline @submit.prevent
            ><el-form-item label="值"
              ><el-input
                v-model="knownValueForm.value"
                class="technical-input"
                placeholder="例如 30" /></el-form-item
            ><el-form-item label="业务含义"
              ><el-input
                v-model="knownValueForm.meaning"
                placeholder="例如 未知 / 离线" /></el-form-item
            ><el-form-item label="排序"
              ><el-input-number
                v-model="knownValueForm.sortOrder"
                :min="0"
                :precision="0"
                controls-position="right" /></el-form-item
            ><el-button type="primary" plain :loading="knownValueSubmitting" @click="addKnownValue"
              >添加</el-button
            ></el-form
          ></el-collapse-item
        >
        <el-collapse-item name="relations"
          ><template #title
            ><span class="drawer-title-with-count"
              >字段级关系 <b>{{ detail.relations.length }}</b></span
            ></template
          >
          <div v-if="detail.relations.length" class="drawer-relation-list">
            <div v-for="item in detail.relations" :key="item.id">
              ...
              ><strong>{{ item.otherObject.title }}</strong>
            </div>
          </div>
          <div v-else class="drawer-empty-state drawer-empty-state--compact">
            <p>尚未建立字段级关系。</p>
      ...
      <p v-if="editError" class="authoring-error column-drawer__mutation-error" role="alert">
        {{ editError }}
      </p>
      <footer class="column-drawer__footer">
        <el-button
          v-if="actorStore.canEdit && detail.canDelete && !editing"
          type="danger"
          plain
          :icon="Delete"
          @click="requestDelete"
        ...
        ><el-button
          ...
          @click="startEditing"
          >编辑字段知识</el-button
        ><el-button v-else-if="editing" @click="stopEditing">结束编辑</el-button
        ><el-button
          v-if="actorStore.canEdit"
          class="skh-section-action skh-evidence-action"
          type="primary"
          ...
          @click="addEvidence"
          >添加证据</el-button
        ><el-button v-if="actorStore.canEdit" :icon="QuestionFilled" @click="createUnknownItem"
          >新建待确认事项</el-button
        >
      </footer>
    </template>
...
.known-value-editor :deep(.el-form-item) {
  margin-bottom: 0;
}
.known-value-editor :deep(.el-input-number) {
  width: 86px;
}
.known-values-list > div {
  grid-template-columns: 88px minmax(0, 1fr) auto;
}
.column-drawer__mutation-error {
  margin: 0 24px 12px;
}
.column-drawer__footer :deep(.el-button) {
  width: 100%;
  height: 42px;
  margin: 0;
}
.column-drawer__footer :deep(.el-icon) {
  font-size: 16px;
}
