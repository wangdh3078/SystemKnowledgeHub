...
const estimatedRowsError = ref<string | null>(null)
const form = reactive({
  businessDescription: '',
...
async function load(): Promise<void> {
  ...
  try {
    ...
    form.estimatedRows = detail.value.metadata.estimatedRows ?? undefined
...
function validateEstimatedRows(): boolean {
  estimatedRowsError.value = null
  if (form.estimatedRows === undefined || form.estimatedRows === null) return true
  if (!Number.isSafeInteger(form.estimatedRows) || form.estimatedRows < 0) {
    estimatedRowsError.value = '请输入 0 至 9007199254740991 之间的整数，或清空此项。'
    return false
  }
  return true
}
...
async function save(): Promise<void> {
  ...
  try {
    ...
    if (error instanceof ApiError) {
      estimatedRowsError.value = error.response.fieldErrors?.estimatedRows?.[0] ?? null
    }
    saveError.value =
  ...
  <div class="column-drawer database-object-edit-drawer">
    ...
    <template v-else-if="detail">
      ...
      <section class="column-drawer__sections database-object-edit-drawer__body">
        ...
        <el-form label-position="top" @submit.prevent>
          ...
          <el-form-item label="业务唯一键"
            ><el-input
              v-model="form.businessKeyColumns"
              class="technical-input"
              placeholder="多个字段用逗号分隔"
          /></el-form-item>
        </el-form>
        <p class="drawer-section-note">
          业务唯一键只能引用当前已登记字段。知识状态、字段、证据和关系均通过各自明确操作维护。
        </p>
      ...
      <footer class="column-drawer__footer">
        ...
        ><el-button type="primary" :loading="saving" @click="save">保存业务知识</el-button>
      </footer>
    </template>
  </div>
</template>
...
<style scoped>
.database-object-edit-drawer__body {
  padding: 18px 24px;
}
