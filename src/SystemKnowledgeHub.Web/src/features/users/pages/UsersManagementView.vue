...
<template>
  <main class="users-page skh-page">
    ...
    <section v-else class="users-table-section skh-table-section" :aria-busy="loading">
      ...
      <el-table v-else :data="data?.items ?? []" row-key="id" class="users-table skh-data-table skh-data-table--comfortable" @row-click="handleRowClick" @sort-change="handleSortChange">
        <el-table-column prop="displayName" label="姓名" min-width="120" sortable="custom"><template #default="scope"><button class="users-table__name skh-table-link" type="button" @click.stop="openEdit(scope.row.id)">{{ scope.row.displayName }}</button></template></el-table-column>
      ...
      <footer v-if="data && data.total > 0" class="users-pagination skh-pagination"><span>{{ (data.page - 1) * data.pageSize + 1 }}–{{ Math.min(data.page * data.pageSize, data.total) }} / {{ data.total }}</span><el-pagination background layout="prev, pager, next" :current-page="data.page" :page-size="data.pageSize" :total="data.total" @current-change="handlePageChange" /></footer>
