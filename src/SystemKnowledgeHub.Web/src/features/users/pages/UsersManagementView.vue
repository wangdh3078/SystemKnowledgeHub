...
function handleSortChange(change: { prop: string; order: 'ascending' | 'descending' | null }): void {
  ...
  const next: UsersSort = change.prop === 'updatedAt'
    ? ascending ? 'updatedAt:asc' : 'updatedAt:desc'
    : ascending ? 'displayName:asc' : 'displayName:desc'
  sort.value = next
  resetPageAndLoad()
}
...
async function toggleUser(user: UserSummary): Promise<void> {
  ...
  try {
    ...
    ElMessage.error(requestError instanceof Error ? requestError.message : `读取用户资料失败，无法${action}。`)
    return
  }
...
<template>
  <main class="users-page">
    ...
    <section class="users-filter-bar" aria-label="用户筛选">
    ...
    <ErrorState v-else-if="error && !data" title="用户列表加载失败" :message="error" @retry="load" />
    <section v-else class="users-table-section" :aria-busy="loading">
      <EmptyState v-if="data && data.items.length === 0" title="没有找到用户" description="调整筛选条件，或创建第一个可维护知识的用户。" />
      <el-table v-else :data="data?.items ?? []" row-key="id" class="users-table" @row-click="handleRowClick" @sort-change="handleSortChange">
        <el-table-column prop="displayName" label="姓名" min-width="120" sortable="custom"><template #default="scope"><strong class="users-table__name">{{ scope.row.displayName }}</strong></template></el-table-column>
        <el-table-column prop="employeeNo" label="工号" min-width="105"><template #default="scope"><span :class="{ 'text-muted': !scope.row.employeeNo }" class="technical-text">{{ scope.row.employeeNo ?? '未记录' }}</span></template></el-table-column>
      ...
      <footer v-if="data && data.total > 0" class="users-pagination"><span>{{ (data.page - 1) * data.pageSize + 1 }}–{{ Math.min(data.page * data.pageSize, data.total) }} / {{ data.total }}</span><el-pagination background layout="prev, pager, next" :current-page="data.page" :page-size="data.pageSize" :total="data.total" @current-change="handlePageChange" /></footer>
      <p v-if="error && data" class="users-inline-error">刷新失败：{{ error }}</p>
    </section>

    <Teleport v-if="overlays.currentDrawer?.kind === 'user-management'" defer to="#drawer-feature-content">
      <UserManagementDrawer :user-id="overlays.currentDrawer.id" @saved="load" />
    </Teleport>
    <Teleport v-if="overlays.currentDialog?.kind === 'knowledge-role-management'" defer to="#dialog-feature-content">
