...
const props = withDefaults(
  ...
  { embedded: false, autoEdit: false, unavailable: false },
)
const emit = defineEmits<{
  updated: [document: KnowledgeDocumentDetail]
  unavailable: [documentId: number]
}>()
...
let relationsSequence = 0
let evidenceSequence = 0
let selectionSequence = 0
let disposed = false
let autoEditConsumed = false
const id = computed(() => props.documentId)
const isCurrent = (requestedId: number | null, sequence: number): boolean =>
  !disposed && id.value === requestedId && selectionSequence === sequence
...
async function load(): Promise<void> {
  ...
  try {
    ...
    if (requestSequence === detailLoadSequence && id.value === requestedId) {
      ...
      if (props.autoEdit && !autoEditConsumed) {
...
async function loadRelations(): Promise<void> {
  if (props.unavailable) return
  const requestedId = id.value
  const sequence = ++relationsSequence
  if (requestedId === null) return
  relationsLoading.value = true
  relationsError.value = null
  try {
    const response = await getRelatedKnowledge('KnowledgeDocument', requestedId)
    if (!disposed && id.value === requestedId && sequence === relationsSequence)
      relations.value = response
  } catch (reason: unknown) {
    if (disposed || id.value !== requestedId || sequence !== relationsSequence) return
    relationsError.value = reason instanceof Error ? reason.message : '无法加载关联对象。'
  } finally {
    if (sequence === relationsSequence) relationsLoading.value = false
  }
}
async function loadEvidence(): Promise<void> {
  ...
  try {
    const response = await getEvidenceList('KnowledgeDocument', requestedId)
    if (!disposed && id.value === requestedId && sequence === evidenceSequence)
      evidence.value = response.items
  } catch (reason: unknown) {
    if (disposed || id.value !== requestedId || sequence !== evidenceSequence) return
...
function refreshTraceability(): void {
  if (!props.unavailable) traceabilitySection.value?.refresh()
}
function refreshImpactContext(): void {
  if (!props.unavailable) impactContextSection.value?.refresh()
}
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent('relationship:changed', {
        detail: { subject: { type: 'KnowledgeDocument', id: requestedId } },
      }),
    )
    ElMessage.success('关联已移除。')
  } catch (reason: unknown) {
    if (!isCurrent(requestedId, sequence)) return
    if (reason !== 'cancel' && reason !== 'close')
      relationsError.value = reason instanceof Error ? reason.message : '移除关联失败。'
  }
}
function beginEdit(): void {
  if (!data.value || props.unavailable || isArchived.value || !canEdit.value) return
...
async function requestLeave(): Promise<boolean> {
  return !editing.value || (await confirmDiscard())
}
defineExpose({ requestLeave, finishEdit, beginEdit })
...
function requestDelete(): void {
  ...
  openDeleteDialog(overlayStore, {
    ...
    onDeleted: () => handleUnavailable(current.id),
    onRefresh: () => (id.value === current.id ? load() : Promise.resolve()),
    onUnavailable: () => handleUnavailable(current.id),
  })
}
function handleUnavailable(documentId: number): void {
  if (props.embedded) emit('unavailable', documentId)
  else if (id.value === documentId) void router.push({ name: 'knowledge-documents-list' })
}
...
async function performSave(): Promise<void> {
  ...
  validationErrors.value = {}
  try {
    const response = await updateKnowledgeDocumentContent(current.id, {
    ...
    if (!isCurrent(current.id, sequence)) return
    data.value = response
    refreshTraceability()
    initialEdit.value = {
    ...
    if (isCurrent(current.id, sequence)) saving.value = false
  }
}
async function requestSave(): Promise<void> {
  if (!data.value || !canSave.value) return
  const requestedId = data.value.id
  const sequence = selectionSequence
  if (data.value.lifecycleStatus !== 'Published') {
    if (isCurrent(requestedId, sequence)) await performSave()
    return
  }
  ...
  try {
    ...
    if (isCurrent(requestedId, sequence)) saveConfirming.value = false
  }
}
...
async function transition(target: DocumentLifecycleStatus): Promise<void> {
  ...
  try {
    ...
    const response = await updateKnowledgeDocumentLifecycle(
      current.id,
      target,
...
function handleCurrentRefreshed(event: Event): void {
  ...
  if (document && document.id === id.value && !editing.value) {
...
function eventMatchesDocument(event: Event): boolean {
  if (!(event instanceof CustomEvent) || !event.detail) return false
  const detail: unknown = event.detail
  if (typeof detail !== 'object' || detail === null || !('subject' in detail)) return false
  const subjects = [detail.subject, 'relatedSubject' in detail ? detail.relatedSubject : null]
  return subjects.some(
    (subject) =>
      ...
      'type' in subject &&
      'id' in subject &&
      ((subject.type === 'KnowledgeDocument' && subject.id === id.value) ||
        (subject.type === 'KnowledgeRelation' &&
          relations.value.some((item) => item.id === subject.id))),
  )
}
function handleRelationshipChanged(event: Event): void {
  if (!eventMatchesDocument(event)) return
  void loadRelations()
  refreshTraceability()
  refreshImpactContext()
}
function handleEvidenceChanged(event: Event): void {
  if (!eventMatchesDocument(event)) return
  void loadEvidence()
  refreshTraceability()
}
function handleKnowledgeStatusChanged(event: Event): void {
  if (!eventMatchesDocument(event) || editing.value) return
  void load()
  refreshTraceability()
}
