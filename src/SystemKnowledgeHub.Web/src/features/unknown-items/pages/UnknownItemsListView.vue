...
<template>
  <main class="unknown-list-page">
    ...
    <el-table v-else :data="data.items" class="unknown-list-table" @row-click="openRow">
      <el-table-column label="问题" min-width="330"><template #default="scope"><div class="unknown-question-cell"><strong>{{ scope.row.question }}</strong><small class="technical-text">{{ scope.row.itemCode }} · {{ scope.row.primaryTarget.display }}</small></div></template></el-table-column>
    ...
    <el-pagination v-if="data && data.total > data.pageSize" v-model:current-page="filters.page" :page-size="filters.pageSize" :total="data.total" layout="prev, pager, next" @current-change="load" />
  </main>
</template>
