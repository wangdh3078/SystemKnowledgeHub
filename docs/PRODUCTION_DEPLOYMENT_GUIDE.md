...
- `DatabaseDiscovery` timeouts, limits, queue polling, lease, and heartbeat values are typed and range-validated. Heartbeat must remain shorter than the lease, and `QueuePollIntervalMilliseconds` must not equal `HeartbeatIntervalSeconds × 1000`. The common idle poll interval is 2000 ms; lease/cancellation/recovery semantics remain unchanged.
