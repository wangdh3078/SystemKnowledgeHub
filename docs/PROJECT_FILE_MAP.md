...
### 1.16 Portal composition, anonymous read, Admin composition, and integration — PORTAL-B01～PORTAL-B04
...
| `.../Application/PortalCompositionValidator.cs`、`PortalLimits.cs` | 校验 page/section compatibility、B04 projection/source compatibility、node cycle/depth/subtree move/order 和冻结 hard limits。 | Internal Knowledge Portal / PORTAL-B01 + PORTAL-B04 | 为 fail-closed reads、transactional writes 与 TrustSummary/Traceability 单目标语义提供单一服务器规则边界。 |
| `.../Application/PortalTargetResolver.cs`、`PortalQueries.cs`、`Models/PortalModels.cs` | 批量解析五类 current eligible target，构造 Home/Tree/Page/Search、canonical breadcrumb、站内目标链接与 closed-discriminator sanitized sections。 | Internal Knowledge Portal / PORTAL-B01 + PORTAL-B03 + PORTAL-B04 | 避免 Admin DTO/raw entity/N+1，并让搜索、附件和失效 target/section 按当前 published context fail closed。 |
| `.../Application/PortalB04ProjectionService.cs` | 从 canonical Attachment/Evidence/KnowledgeRelation/KnowledgeDocument 派生 AttachmentList、单目标 TrustSummary、RelatedKnowledge 与 bounded requirement/specification/test trace。 | Internal Knowledge Portal / PORTAL-B04 | 以 request-scoped bounded query/cache 读取现有事实，不复制知识、聚合多目标 trust 或建立第二套 relation/trace truth。 |
| `.../Api/PortalController.cs` | 暴露匿名 GET Home、Tree、Page 与分页 Search。 | Internal Knowledge Portal / PORTAL-B01 + PORTAL-B03 + PORTAL-B04 | 在不降低全局 Viewer/Admin authorization 的情况下建立专用 published-only read/search boundary。 |
| `.../Api/PortalAttachmentsController.cs` | 以 page-scoped authorization 暴露匿名 attachment content/download/preview，并复用 canonical 存储完整性与安全预览。 | Internal Knowledge Portal / PORTAL-B04 | 防止仅凭附件 ID 越权、历史 revision/未发布页面泄漏及 StorageKey/hash/token 暴露。 |
| `.../Application/AdminPortalService.cs`、`AdminPortalQueries.cs`、`PortalPublicationReadiness.cs`、`Models/AdminPortalModels.cs` | 实现 Administrator whole-page composition、tree mutation/publication、五类 target picker、八类 projection 的 Admin inventory/preview 与中文发布阻塞检查。 | Admin Knowledge Composition / PORTAL-B02 + PORTAL-B04 | 复用同一 resolver/projector，保持 opaque concurrency、原子 sibling reorder、fail-closed reference health 与 canonical knowledge write isolation。 |
...
### 2.20 Admin Knowledge Composition and Portal Reading UX — PORTAL-B02～PORTAL-B04

| 路径 | 一句话职责 | Feature / Vertical Slice | 为什么需要 |
| --- | --- | --- | --- |
| `.../features/portal-management/api/portalManagementContracts.ts`、`portalManagementApi.ts` | 严格解码 Admin Portal 页面、树、readiness、五类 target picker 与八类统一 preview projection，并调用 typed `/api/admin/portal/**`。 | Admin Knowledge Composition / PORTAL-B02 + PORTAL-B04 | 让管理端 fail closed 消费 safe DTO、opaque token 与标准错误，不暴露 raw entity/version/audit FK。 |
| `.../features/portal-management/pages/PortalManagementView.vue`、`components/PortalTargetPickerDialog.vue`、`PortalPreviewDialog.vue`、`portal-management.css` | 提供 Page Tree、whole-page section 编排、B04 source/projection compatibility、受控知识选择、发布检查及复用阅读 renderer 的 Preview。 | Admin Knowledge Composition / PORTAL-B02 + PORTAL-B04 | 让管理员能安全编排 Attachment/Trust/Related/Trace sections，同时保持 dirty guard、发布边界与 canonical knowledge isolation。 |
...
| `.../features/portal-reading/api/portalReadContracts.ts`、`portalReadApi.ts` | 严格解码匿名 Home/Tree/Page/Search、八类 section DTO 与 page-scoped attachment URL，并以 `credentials: omit` 的 GET-only client 访问 `/api/portal/**`。 | Internal Knowledge Portal / PORTAL-B03 + PORTAL-B04 | 让 Portal Search/附件/信任/关系/追溯不依赖 antiforgery、Current User、Admin 401 handler 或 canonical Admin APIs。 |
| `.../features/portal-reading/pages/PortalHomeView.vue`、`PortalPageView.vue`、`PortalSearchView.vue`、`PortalNotFoundView.vue`、`components/PortalTreeNavigation.vue`、`PortalSectionRenderer.vue` | 提供匿名首页/搜索/页面与八类 closed renderer，包括受控正文图片、附件、单目标信任、站内相关知识和 bounded trace。 | Internal Knowledge Portal / PORTAL-B03 + PORTAL-B04 | 复用安全 Markdown/Mermaid，只渲染 safe DTO 与 Portal link，阻止 raw JSON、任意 URL、StorageKey 与管理操作泄漏。 |
...
| `.../Api/PortalAnonymousReadApiTests.cs`、`Application/PortalCompositionValidatorTests.cs`、`Application/PortalQueryPerformanceTests.cs` | 覆盖 Home/Tree/Page/Search、publication/lifecycle、附件 page-scope、四类 B04 projection、trace 语义/limits、sanitization、hard limits 与 B04 repeated-section 固定 query count。 | Internal Knowledge Portal / PORTAL-B01 + PORTAL-B03 + PORTAL-B04 | 保护 Portal published-only fail-closed contract、搜索精确分页、canonical truth projection 与 bounded query safety。 |
| `.../Api/AdminPortalApiTests.cs` | 以真实 task-owned SQLite/HTTP 覆盖 Administrator/antiforgery、whole-page replacement、八类 projection Preview/readiness、发布、树、并发、失效引用和 canonical knowledge/relation isolation。 | Admin Knowledge Composition / PORTAL-B02 + PORTAL-B04 | 保护 Admin composition 高风险写入/发布边界、B04 compatibility、原子性和匿名 visibility contract。 |
...
| `.../src/features/portal-management/api/portalManagementApi.spec.ts`、`portalManagementSurface.spec.ts`、`src/layouts/AppSidebar.attachments.spec.ts` | 覆盖 Admin Portal strict API、树/页面/八类章节/picker/同投影 preview/publish UX、B04 compatibility、dirty/conflict/broken-reference 与 Administrator-only 导航。 | Admin Knowledge Composition / PORTAL-B02 + PORTAL-B04 | 保护管理工作台的受控组合、TrustSummary/Traceability source 边界、opaque concurrency 和角色可见性。 |
| `.../src/features/portal-reading/**/*.spec.ts`、`src/layouts/PortalLayout.spec.ts`、`src/app/router/portalRouteSecurity.spec.ts`、`src/App.spec.ts` | 覆盖匿名 GET client、Home/Tree/Page/Search、受控图片与附件链接、八类 closed renderer、loading/error/paging、Portal shell 与 SecurityGate/Current User bypass。 | Internal Knowledge Portal / PORTAL-B03 + PORTAL-B04 | 保护匿名阅读体验、strict safe DTO/URL 与 B04 交互不泄漏登录/Admin UI，同时保持 Admin 路由认证授权。 |
...
| `docs/design/PORTAL_A01_INTERNAL_KNOWLEDGE_PORTAL_ARCHITECTURE_DECISION.md`、`docs/reports/PORTAL_A01_INTERNAL_KNOWLEDGE_PORTAL_ARCHITECTURE_FREEZE_REPORT.md` 与 `docs/reports/PORTAL_A01_AMEND_01_TRUSTSUMMARY_SOURCE_COMPATIBILITY_REPORT.md` | 冻结并验证同应用、同部署的匿名只读 Portal、Admin composition 边界，以及 TrustSummary 仅允许单一 PrimaryTarget/ExplicitReference、禁止 Derived 聚合的显式 amendment。 | Internal Knowledge Portal / Admin Knowledge Composition / PORTAL-A01 / PORTAL-A01-AMEND-01 | 是 PORTAL-B01～VERIFY 的直接架构与合同依据；任何实现不得复制知识事实、聚合多目标 trust 或放宽现有 Admin 授权。 |
...
| `docs/reports/PORTAL_B04_SEARCH_ATTACHMENT_TRUST_RELATED_TRACE_INTEGRATION_VERIFICATION_REPORT.md` | 记录 published-only Search、page-scoped attachment delivery、四类 canonical projection、Admin 同投影 Preview、浏览器、security、migration、cleanup 与 repository data protection 证据。 | Internal Knowledge Portal / Admin Knowledge Composition / PORTAL-B04 | 为 B04 phase integration 完成状态和 PORTAL-VERIFY readiness 提供可复核证据。 |
| `docs/reports/PORTAL_FINAL_VERIFICATION_REPORT.md` | 记录当前 HEAD focused tests、隔离 Admin 编排到匿名阅读、HC 撤销、取消发布 fail-closed、model、DBSAFE 与清理的最终验收。 | Internal Knowledge Portal / PORTAL-VERIFY — PASS / v1 COMPLETE | Portal phase 完成依据；SEC-04 Production rollout 与 PHASE-TRACE real-domain Product acceptance 保持独立。 |
| `docs/design/REL_EVIDENCE_A01_RELATION_EVIDENCE_DEPENDENCY_AND_REMOVAL_DECISION.md` 与 `docs/reports/REL_EVIDENCE_A01_RELATION_EVIDENCE_DEPENDENCY_DECISION_REPORT.md` | 冻结关系已有 Evidence/HC 时阻止物理移除、422 契约、原子检查与后续验收，并记录 STABILITY-R01 合同缺口关闭。 | Relationships / REL-EVIDENCE-A01 | 为 STABILITY-R01-R01 提供批准依据；不代表运行时修复已实现。 |
| `docs/reports/STABILITY_R01_R01_RELATION_EVIDENCE_DEPENDENCY_IMPLEMENTATION_VERIFICATION_REPORT.md` | 记录 Relation Evidence/HC 原子依赖阻止、真实并发、回归与 DBSAFE 验证，关闭 STABILITY-R01 剩余实现缺口。 | Stability / STABILITY-R01-R01 | 为 STABILITY-R01 完成和 STABILITY-R02 readiness 提供验证依据。 |
