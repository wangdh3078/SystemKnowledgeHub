...
- **Status / severity:** CLOSED — REV-GAP-012-R01; Low; recorded during STABILITY-R01 on 2026-09-05, closed on 2026-09-08.
- **Owner / area:** Backend migration-test maintenance.
- **Original reproduction:** `DatabaseDiscoverySyncMigrationTests.B04_migration_preserves_legacy_rows_backfills_technical_identity_and_adds_typed_sync_tables` calls parameterless `migrator.MigrateAsync()` and then asserts an exact five-table B04 delta.
- **Original actual:** Latest includes the approved `20260903142533_AddPortalCompositionFoundation`; the extra tables are `portal_page_nodes`, `portal_page_sections`, and `portal_pages`. The exact-table assertion fails. The test and migrations are unchanged from starting HEAD `0ba5eb6`.
- **Original impact:** The supplemental migration test is FAIL; it is not evidence of a schema/data regression caused by STABILITY-R01. No STABILITY-R01 schema/migration changes exist. A full backend suite must not be described as all passing while this remains open.
- **Original recommendation:** Explicitly test the B04 migration boundary, and keep latest-chain preservation coverage separate. Do not remove legitimate Portal tables or weaken production migrations to satisfy an obsolete test expectation.
- **Closure — REV-GAP-012-R01:** The test now declares `TargetMigration = "20260831170031_AddManualDiscoverySyncFoundation"` and explicitly migrates from `20260830030122_AddDatabaseDiscoveryRunSnapshotDiffFoundation` to that B04 target, then back to the predecessor. It no longer upgrades to repository latest. The unchanged exact five-table assertion therefore verifies only the B04 delta; subsequent Portal, HC lifecycle and future migrations do not enter this test boundary.
- **Verification:** `dotnet test tests/SystemKnowledgeHub.Api.Tests -c Release --no-restore --disable-build-servers --filter "FullyQualifiedName~DatabaseDiscoverySyncMigrationTests" -- xUnit.ParallelizeTestCollections=false` — **PASS, 1/1; 0 failed, 0 skipped**. Existing row counts, technical-identity backfill, business fields, unique indexes, provider-neutral tables, foreign-key check and Down/original-schema assertions all remain intact. `git diff --check`: PASS.
- **Scope / safety:** Test-boundary correction only; no product/model/migration changes or new migration. The test uses `Data Source=:memory:;Foreign Keys=True`; no repository SQLite/WAL/SHM was accessed. No runtime server, provider or frontend/browser verification was started; the one-shot test process exited. Other REV gap statuses remain unchanged.
- **PORTAL-VERIFY:** READY YES; not executed. No Analysis Workspace work was started.
